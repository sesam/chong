...
export function i18nGatePrompt(summary: string): string {
  return [
    ...
    "- Filling empty msgstr in .en.po / .sl.po (clear source string; English msgid → fill .en.po with msgid; .sl.po from translation-memory / neighbors, or msgid when nearby England-planning strings stay English)",
    "- Resolving identical en/sl msgstr by translating .sl.po OR adding intentional matches to scripts/i18n-identical-msgstr-allowlist.json (England planning jargon, brands, proper nouns — match existing allowlist style, keep lists sorted)",
...
export function i18nResolvePrompt(summary: string, i18nCmd: string): string {
  return [
    ...
    "- Empty msgstr: fill .en.po with the English msgid; for .sl.po prefer translation-memory.csv / neighboring .po entries, else use the msgid when that matches nearby England-planning strings.",
    "- Identical en/sl after filling: give .sl.po a real Slovenian msgstr, OR add the msgid to scripts/i18n-identical-msgstr-allowlist.json (sorted msgids list) when keeping English is intentional — see that file's _comment and existing England planning entries.",
